...
def display_histogram(image_path, title, window, key):
    ...
    layout = [[sg.Canvas(key=key)], [sg.Text(title, size=(20, 1))]]
...
layout = [
    ...
    [
        ...
        sg.InputText(key="input_file", do_not_clear=False),
        sg.FileBrowse(),
    ],
    [sg.Button("Encrypt"), sg.Button("Decrypt")],
    [sg.Text("", key="status_text")],  # Add a text element to display the status
    [
        sg.Image(size=(150, 150), key="-ORIGINAL-"),
        sg.Image(size=(150, 150), key="-ENCRYPTED-"),
        sg.Image(size=(150, 150), key="-DECRYPTED-"),
    ],
    [
        sg.Text("Original Image", size=(20, 1)),
        sg.Text("Encrypted Image", size=(20, 1)),
        sg.Text("Decrypted Image", size=(20, 1)),
    ],
...
def update_images(original_path, encrypted_path, decrypted_path):
    try:
        ...
        window["-ORIGINAL-"].update(
            data=None
        )  # Handle the case when the image cannot be opened
    ...
    if event == sg.WIN_CLOSED:
        ...
        if input_file:
            ...
            window["status_text"].update(
                "Image encrypted and saved as encrypted_image.jpg, please select an image to decrypt"
            )
        ...
        if input_file:
            ...
            display_histogram(
                "image.jpg", "Original Image Histogram", window, "-HIST_ORIGINAL-"
            )
            display_histogram(
                "encrypted_image.bmp",
            ...
            horizontal_correlation_result = calculate_horizontal_correlation(
                np.array(Image.open("image.jpg").convert("L"))
            )
            vertical_correlation_result = calculate_vertical_correlation(
                np.array(Image.open("encrypted_image.bmp").convert("L"))
            )
            diagonal_kernel = np.array([[2, -1, -1], [-1, 2, -1], [-1, -1, 2]])
            diagonal_correlation_result = calculate_diagonal_correlation(
                np.array(Image.open("decrypted_image.jpg").convert("L")),
                diagonal_kernel,
            )
            ...
            window["-ENTROPY-"].update(
                f"Entropy Comparison: Original - {entropy_original:.2f}, Encrypted - {entropy_encrypted:.2f}"
            )

            window["-HORIZONTAL-"].update(
                f"Horizontal Correlation: {horizontal_correlation_result:.2f}"
            )
            window["-VERTICAL-"].update(
                f"Vertical Correlation: {vertical_correlation_result:.2f}"
            )
            window["-DIAGONAL-"].update(
                f"Diagonal Correlation: {diagonal_correlation_result:.2f}"
            )
